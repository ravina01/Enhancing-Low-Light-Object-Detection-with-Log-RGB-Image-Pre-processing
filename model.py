"""
Ravina Lad
This file contains classes like - 
RegionProposalNetwork, Classification model, resnet 50 - FeatureExtractor model.
TwoStageDetector - that puts together the region proposal network and the classification module to build the final end-to-end Faster-RCNN model.
"""
...
class ProposalModule(nn.Module):
    def __init__(self, in_features, hidden_dim=512, n_anchors=9, p_dropout=0.4):
        super().__init__()
    ...
    def inference(self, images, conf_thresh=0.5, nms_thresh=0.7):
        with torch.no_grad():
            ...
            for i in range(batch_size):
                ...
                conf_scores_pos = conf_scores_pos.to('cuda')
                proposals_pos = proposals_pos.to('cuda')
...
class ClassificationModule(nn.Module):
    def __init__(self, out_channels, n_classes, roi_size, hidden_dim=512, p_dropout=0.4):
        super().__init__()        
        ...
        self.dropout = nn.Dropout(p_dropout)
        self.fc = self.fc.to('cuda')
        # define classification head
        self.cls_head = nn.Linear(hidden_dim, n_classes)
        self.cls_head = self.cls_head.to('cuda')
    ...
    def forward(self, feature_map, proposals_list, gt_classes=None):
        ...
        for i in range(len(proposals_list)):
            proposals_list[i] = proposals_list[i].to('cuda')
...
def calc_bbox_reg_loss(gt_offsets, reg_offsets_pos, batch_size):
    ...
    reg_offsets_pos = reg_offsets_pos.to('cuda')
    gt_offsets = gt_offsets.to('cuda')
